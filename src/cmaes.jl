...
struct CMAES{T} <: AbstractOptimizer
    ...
    function CMAES(; μ::Int=5, λ::Int=2μ, mu::Int=μ, lambda::Int=0, weights::Vector{T}=zeros(lambda),
...
function initial_state(method::CMAES, options, objfun, population)
    ...
    if !(1 ≤ μ_eff ≤ μ)
        ...
        # TODO: need better weights initialization
        # wᵢ = [ifelse(w≥ 0, w/w′⁺, min(α⁻,α⁻_eff, α⁻_pd)*w/α⁻) for w in w′ᵢ]
        wᵢ = [ifelse(w≥ 0, w/w′⁺, 0.0) for w in w′ᵢ]
    else
