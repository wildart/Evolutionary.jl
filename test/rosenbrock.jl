@testset "Rosenbrock" begin
    ...
    @testset "ES settings" for (sn,ss) in settings, sel in selections
        ...
        result = Evolutionary.optimize( rosenbrock, (() -> rand(N)), m, Evolutionary.Options(iterations=1000, successive_f_tol=25, abstol=1e-5, rng=rng))
    ...
    m = ES( initStrategy = IsotropicStrategy(N),
                ...
                μ = 15, ρ = 3, λ = 100)
    result = Evolutionary.optimize(rosenbrock, BoxConstraints(0.0, 0.5, N), (() -> rand(N)), m, Evolutionary.Options(rng=StableRNG(1)))
    ...
    Random.seed!(rng, 42)
    result = Evolutionary.optimize(rosenbrock, (() -> rand(Float32,N)), CMAES(mu = 5, lambda = 100, weights=zeros(Float32,100)), opts)
    ...
    result = Evolutionary.optimize(rosenbrock, bc, (() -> rand(N)), CMAES(mu = 5, lambda = 100), opts)
    ...
    result = Evolutionary.optimize(rosenbrock, c, (() -> rand(5N)), CMAES(mu = 40, lambda = 100), opts)
    ...
    result = Evolutionary.optimize(rosenbrock, c, (() -> rand(2N)), CMAES(mu = 8, lambda = 100), opts)
    println("(5/5,100)-CMA-ES [penalty] => F: $(minimum(result)), C: $(Evolutionary.iterations(result))")
    @test Evolutionary.minimizer(result) |> sum ≈ 1.0 atol=0.1
    ...
    result = Evolutionary.optimize(rosenbrock, (() -> rand(N)), m, Evolutionary.Options(rng=StableRNG(1)))
    ...
    # Testing: DE
    result = Evolutionary.optimize(rosenbrock, (() -> rand(N)), DE(populationSize = 100), Evolutionary.Options(rng=StableRNG(1)))
